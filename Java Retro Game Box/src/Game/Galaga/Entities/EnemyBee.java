...
public class EnemyBee extends BaseEntity {
	int row,col;//row 3-4, column 0-7
	boolean justSpawned=true,attacking=true, positioned=false,hit=false,centered = false;
	...
	int timeAlive=0;
	int spawnCoolDown = random.nextInt(60*20)+60*5;
	boolean moving = true;
	int playerPosx = handler.getGalagaState().entityManager.playerShip.x;
	...
	public void tick() {
		...
		if (justSpawned){
		...
		}else if (positioned && handler.getGalagaState().entityManager.playerShip.initSpawn==false){
			justSpawned = false;
			spawnCoolDown--;
			if (spawnCoolDown<=0) {
				if (x>playerPosx && moving) {
					x-=2;
					if(x==playerPosx) {
						moving = false;
						positioned = false;
			...
			if(y!=playerPosy) {
				y+=2;
				if(y>playerPosy || y==playerPosy) {
					enemyDeath.tick();
					remove = true;
					handler.getGalagaState().entityManager.playerShip.entitySpawns[row][col]=0;
					handler.getGalagaState().entityManager.playerShip.beeSpawnCD++;
					handler.getGalagaState().entityManager.playerShip.allowBeeSpawn=true;
				}
			}
